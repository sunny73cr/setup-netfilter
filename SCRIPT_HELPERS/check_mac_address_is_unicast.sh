...
print_description() {
	printf "A program that confirms or denies if a MAC address is a 'unicast' address.\n">&2;
}
...
MASK_FIRST_OCTET=$(( $FIRST_OCTET_DECIMAL&1 ));

if [ "$MASK_FIRST_OCTET" -eq 0 ]; then
#the least significant bit in the first octet is not set.
	exit 0;
else
	exit 1;
fi
