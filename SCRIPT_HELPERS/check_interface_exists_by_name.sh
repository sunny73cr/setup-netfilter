...
print_description() {
	printf "A program that confirms if an interface exists matching the provided name.\n">&2;
}
...
print_dependencies() {
	...
	printf "grep\n">&2;
	printf "find\n">&2;
	printf "read permission for the /sys/class/net directory\n">&2;
	printf "\n">&2;
}
...
print_usage() {
	...
	printf "Parameters:\n">&2;
	printf "\n">&2;
	printf " Required: --name x\n">&2;
	printf "  (https://github.com/torvalds/linux/blob/master/net/core/dev.c)\n">&2;
	printf "  where x is 1-15 characters,\n">&2;
	printf "  does not contain '/', ':',\n">&2;
	printf "  does not contain whitespace (space, horizontal tab, line feed, vertical tab, form feed),\n">&2;
	printf "\n">&2;
...
if [ $SKIP_VALIDATION -eq 0 ]; then
	...
	#1-15 character strings beginning with a letter.
	if [ -z "$(echo $NAME | grep '[a-zA-Z][a-zA-Z0-9]\{0,14\}')" ]; then
		printf "\nInvalid --name. ">&2;
		print_usage_then_exit;
	fi
fi
...
INTERFACE_DESCRIPTIONS_FILTERED_BY_NAME=$(find '/sys/class/net/' | grep "^/sys/class/net/$NAME$");
