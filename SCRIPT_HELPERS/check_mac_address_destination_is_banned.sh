...
print_description() {
	printf "A program that confirms if a MAC address is banned as a destination.\n">&2;
}
...
print_dependencies() {
	printf "printf\n">&2;
	printf "$DEPENDENCY_PATH_CHECK_MAC_ADDRESS_IS_VALID\m">&2;
	printf "\n">&2;
}
...
print_usage() {
	...
	printf "  The MAC address to validate. If the address is invalid, an exit code of 2 is returned.\n">&2;
	printf "\n">&2;
...
if [ "$ADDRESS" = "00:00:00:00:00" ]; then
