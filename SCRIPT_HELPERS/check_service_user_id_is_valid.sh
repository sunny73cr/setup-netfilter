...
print_description() {
	printf "A program that confirms or denies if a user ID is a valid user ID.\n">&2;
}
...
print_usage() {
	...
	printf "Parameters:\n">&2;
	printf "\n">&2;
	printf " Required: --id X (where X is 0-65535)\n">&2;
	printf "  The ID of the user that provides the network service.\n">&2;
	printf "\n">&2;
...
if [ $SKIP_VALIDATION -eq 0 ]; then
	...
	if [ -z "$(echo $SERVICE_USER_ID | grep '^[0-9]\{1,5\}$')" ]; then
		printf "\nInvalid --id (must be a 1-5 digit number). ">&2;
		print_usage_then_exit;
	fi

	if [ $SERVICE_USER_ID -lt 0 ]; then
		printf "\nInvalid --id (must be greater than or equal to 0). ">&2;
		print_usage_then_exit;
	fi

	if [ $SERVICE_USER_ID -gt 65535 ]; then
		printf "\nInvalid --id (must be less than 65536). ">&2;
		print_usage_then_exit;
	fi
fi
...
RES="$(cat /etc/passwd | cut -d ':' -f 3 | grep $SERVICE_USER_ID)";

if [ "$RES" = "$SERVICE_USER_ID" ]; then
	exit 0;
else
