...
print_description() {
	printf "A program that confirms or denies if a MAC address is valid.\n">&2;
}
...
print_usage() {
	...
	printf "  The MAC address string to check.\n">&2;
	printf "  If the MAC address does not follow the form; an exit code of 1 is returned.\n">&2;
	printf "\n">&2;
}
...
if [ -z "$(echo $ADDRESS | grep '[0-9a-fA-F]\{2\}:[0-9a-fA-F]\{2\}:[0-9a-fA-F]\{2\}:[0-9a-fA-F]\{2\}:[0-9a-fA-F]\{2\}:[0-9a-fA-F]\{2\}')" ]; then
	exit 1;
fi
