...
DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN_SET="./TCP/try_match_tcp_flags_fin_set.sh";

if [ ! -x "$DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN_SET" ]; then
	echo "$0; dependency script failure: \"$DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN_SET\" is missing or is not executable.">&2;
	exit 3;
fi
...
CMD_TRY_MATCH_TCP_FLAGS_FIN_SET="$DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN_SET";
$CMD_TRY_MATCH_TCP_FLAGS_FIN_SET;
check_success;
