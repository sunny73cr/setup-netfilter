...
DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN="./TCP/try_match_tcp_fin.sh";

if [ ! -x "$DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN" ]; then
	echo "$0; script dependency failure: \"$DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN\" is missing or is not exectuable.";
	exit 3;
fi
...
CMD_TRY_MATCH_TCP_FIN="$DEP_SCRIPT_PATH_TRY_MATCH_TCP_FIN";
if [ -n "$CLIENT_PORT" ]; then
	CMD_TRY_MATCH_TCP_FIN="$CMD_TRY_MATCH_TCP_FIN --source-port $SERVER_PORT";
fi
if [ -n "$SERVER_PORT" ]; then
	CMD_TRY_MATCH_TCP_FIN="$CMD_TRY_MATCH_TCP_FIN --destination-port $CLIENT_PORT";
fi
if [ -n "$SERVICE_UID" ]; then
	CMD_TRY_MATCH_TCP_FIN="$CMD_TRY_MATCH_TCP_FIN --service-user-id $SERVICE_UID";
fi
PROTOCOL_MATCH=$($CMD_TRY_MATCH_TCP_FIN);
check_success;
