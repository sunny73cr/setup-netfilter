...
DEP_SCRIPT_PATH_TRY_MATCH_TCP_ACK="./TCP/try_match_tcp_ack.sh";

if [ ! -x "$DEP_SCRIPT_PATH_TRY_MATCH_TCP_ACK" ]; then
	echo "$0; script dependency failure: \"$DEP_SCRIPT_PATH_TRY_MATCH_TCP_ACK\" is missing or is not exectuable.";
	exit 3;
fi
...
CMD_TRY_MATCH_TCP_ACK="$DEP_SCRIPT_PATH_TRY_MATCH_TCP_ACK";
if [ -n "$CLIENT_PORT" ]; then
	CMD_TRY_MATCH_TCP_ACK="$CMD_TRY_MATCH_TCP_ACK --source-port $SERVER_PORT";
fi
if [ -n "$SERVER_PORT" ]; then
	CMD_TRY_MATCH_TCP_ACK="$CMD_TRY_MATCH_TCP_ACK --destination-port $CLIENT_PORT";
fi
if [ -n "$SERVICE_UID" ]; then
	CMD_TRY_MATCH_TCP_ACK="$CMD_TRY_MATCH_TCP_ACK --service-user-id $SERVICE_UID";
fi
PROTOCOL_MATCH=$($CMD_TRY_MATCH_TCP_ACK);
check_success;
