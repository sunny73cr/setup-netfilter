...
DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST="./TCP/try_match_tcp_rst.sh";

if [ ! -x "$DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST" ]; then
	echo "$0; script dependency failure: \"$DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST\" is missing or is not exectuable.";
	exit 3;
fi
...
CMD_TRY_MATCH_TCP_RST="$DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST";
if [ -n "$CLIENT_PORT" ]; then
	CMD_TRY_MATCH_TCP_RST="$CMD_TRY_MATCH_TCP_RST --source-port $SERVER_PORT";
fi
if [ -n "$SERVER_PORT" ]; then
	CMD_TRY_MATCH_TCP_RST="$CMD_TRY_MATCH_TCP_RST --destination-port $CLIENT_PORT";
fi
if [ -n "$SERVICE_UID" ]; then
	CMD_TRY_MATCH_TCP_RST="$CMD_TRY_MATCH_TCP_RST --service-user-id $SERVICE_UID";
fi
PROTOCOL_MATCH=$($CMD_TRY_MATCH_TCP_RST);
check_success;
