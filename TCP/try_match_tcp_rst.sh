...
DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST_SET="./TCP/try_match_tcp_flags_rst_set.sh";

if [ ! -x "$DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST_SET" ]; then
	echo "$0; dependency script failure: \"$DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST_SET\" is missing or is not executable.">&2;
	exit 3;
fi
...
echo "\t\tct state new, established \\";

CMD_TRY_MATCH_TCP_FLAGS_RST_SET="$DEP_SCRIPT_PATH_TRY_MATCH_TCP_RST_SET";
$CMD_TRY_MATCH_TCP_FLAGS_RST_SET;
check_success;
