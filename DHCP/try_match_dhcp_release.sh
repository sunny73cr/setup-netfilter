...
print_usage() {
	...
	printf "\n">&2;
	printf " Optional: --released-ipv4-address X.X.X.X (where X is 0-255)\n">&2;
	printf "  Note: the IPV4 Address released to the network client.\n">&2
	printf "\n">&2
	printf " Optional: --released-ipv4-network X.X.X.X/Y (where X is 0-255, and Y is 1-32)\n">&2
	printf "  Note: a contiguous block of IPV4 Addresses that could be released to the network client.\n">&2
	printf "\n">&2
	printf " Note: you cannot combine --released-ipv4-address and --released-ipv4-network\n">&2;
	printf "\n">&2;
	printf " Note: it is strongly recommended to supply a released address or network.\n">&2;
	printf "\n">&2;
	...
	printf "  Note: it is stronly recommended to supply the user ID that is released to the DHCP server 'service' listed in the /etc/passwd file.\n">&2;
	printf "  Note: without this restriction, DHCP RELEASE packets are permitted to any program.\n";
	printf "\n">&2;
...
while true; do
	case $1 in
		...
		--released-ipv4-network)
			if [ $# -lt 2 ]; then
				print_usage_then_exit;
			elif [ -z "$2" ]; then
				print_usage_then_exit;
			else
				RELEASED_IPV4_NETWORK=$2;
				shift 2;
			fi
		;;
...
if [ $SKIP_VALIDATION -eq 0 ]; then
	...
	if [ -n "$RELEASED_IPV4_ADDRESS" ] && [ -n "$RELEASED_IPV4_NETWORK" ]; then
		printf "\nInvalid combination of --released-ipv4-address and --released-ipv4-network. ">&2;
		print_usage_then_exit;
	fi

	if [ -n "$RELEASED_IPV4_ADDRESS" ]; then
		$DEPENDENCY_PATH_CHECK_IPV4_ADDRESS_IS_VALID --address $RELEASED_IPV4_ADDRESS;
		case $? in
			0) ;;
			1) printf "\nInvalid --released-ipv4-address. \n">&2; print_usage_then_exit; ;;
			*) printf "$0: dependency: \"$DEPENDENCY_PATH_CHECK_IPV4_ADDRESS_IS_VALID\" produced a failure exit code ($?).\n">&2; exit 3; ;;
		esac
	fi

	if [ -n "$RELEASED_IPV4_NETWORK" ]; then
		$DEPENDENCY_PATH_CHECK_IPV4_NETWORK_IS_VALID --network $RELEASED_IPV4_NETWORK;
		case $? in
			0) ;;
			1) printf "\nInvalid --released-ipv4-network. \n">&2; print_usage_then_exit; ;;
...
RELEASED_IPV4_ADDRESS_DECIMAL="";
if [ -n "$RELEASED_IPV4_ADDRESS" ]; then
	RELEASED_IPV4_ADDRESS_DECIMAL=$($DEPENDENCY_PATH_CONVERT_IPV4_ADDRESS_TO_DECIMAL --address $RELEASED_IPV4_ADDRESS);
	case $? in
		0) ;;
...
RELEASED_IPV4_NETWORK_BASE_ADDRESS="";
RELEASED_IPV4_NETWORK_BASE_ADDRESS_DECIMAL="";
RELEASED_IPV4_NETWORK_END_ADDRESS="";
RELEASED_IPV4_NETWORK_END_ADDRESS_DECIMAL="";
if [ -n "$RELEASED_IPV4_NETWORK" ]; then
	RELEASED_IPV4_NETWORK_BASE_ADDRESS=$($DEPENDENCY_PATH_CONVERT_CIDR_NETWORK_TO_BASE_ADDRESS --network $RELEASED_IPV4_NETWORK);
	case $? in
		0) ;;
		*) printf "$0: dependency \"$DEPENDENCY_PATH_CONVERT_CIDR_NETWORK_TO_BASE_ADDRESS\" produced a failure exit code ($?).">&2; exit 3; ;;
	esac

	RELEASED_IPV4_NETWORK_BASE_ADDRESS_DECIMAL=$($DEPENDENCY_PATH_CONVERT_IPV4_ADDRESS_TO_DECIMAL --address $RELEASED_IPV4_NETWORK_BASE_ADDRESS);
	case $? in
		0) ;;
		*) printf "$0: dependency \"$DEPENDENCY_PATH_CONVERT_IPV4_ADDRESS_TO_DECIMAL\" produced a failure exit code ($?).">&2; exit 3; ;;
	esac

	RELEASED_IPV4_NETWORK_END_ADDRESS=$($DEPENDENCY_PATH_CONVERT_CIDR_NETWORK_TO_END_ADDRESS --network $RELEASED_IPV4_NETWORK);
	case $? in
		0) ;;
		*) printf "$0: dependency \"$DEPENDENCY_PATH_CONVERT_CIDR_NETWORK_TO_END_ADDRESS\" produced a failure exit code ($?).">&2; exit 3; ;;
	esac

	RELEASED_IPV4_NETWORK_END_ADDRESS_DECIMAL=$($DEPENDENCY_PATH_CONVERT_IPV4_ADDRESS_TO_DECIMAL --address $RELEASED_IPV4_NETWORK_END_ADDRESS);
	case $? in
		0) ;;
		*) printf "$0: dependency \"$DEPENDENCY_PATH_CONVERT_IPV4_ADDRESS_TO_DECIMAL\" produced a failure exit code ($?).">&2; exit 3; ;;
	esac
...
printf "\\t#Flags: no flags set for DHCP RELEASE, 16 zeroes, DHCPRELEASE is not broadcasted (See RFC1541)\n";
...
if [ -n "$RELEASED_IPV4_ADDRESS" ]; then
	printf "\\t\\t@$OFFSET_MARKER,$BIT_OFFSET_CLIENT_IP_ADDRESS,32 $RELEASED_IPV4_ADDRESS_DECIMAL \\\\\n";
fi

if [ -n "$RELEASED_IPV4_NETWORK_BASE_ADDRESS_DECIMAL" ]; then
	printf "\\t\\t@$OFFSET_MARKER,$BIT_OFFSET_CLIENT_IP_ADDRESS,32 >= $RELEASED_IPV4_NETWORK_BASE_ADDRESS_DECIMAL \\\\\n";
fi

if [ -n "$RELEASED_IPV4_NETWORK_END_ADDRESS_DECIMAL" ]; then
	printf "\\t\\t@$OFFSET_MARKER,$BIT_OFFSET_CLIENT_IP_ADDRESS,32 <= $RELEASED_IPV4_NETWORK_END_ADDRESS_DECIMAL \\\\\n";
fi

if [ -z "$RELEASED_IPV4_ADDRESS" ] && [ -z "$RELEASED_IPV4_NETWORK" ]; then
...
printf "\\t#Match GIADDR (Gateway/Relay Agent IP address)\n";
