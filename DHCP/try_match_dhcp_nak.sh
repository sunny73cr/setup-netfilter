...
print_description() {
	printf "A program that prints part of an NFT rule 'match' section. The match intends to identify DHCP NAK packets.\n">&2;
}
...
print_usage() {
	...
	printf "  Note: without this restriction, DHCP NAK packets are permitted to any program.\n";
	printf "\n">&2;
...
printf "\\t#Flags: Broadcast flag set for DHCPNAK, 1 bit on, 15 bits off, DHCNAK is broadcasted (See RFC1541)\n";
...
printf "\\t#DHCP Message Type of 6 (Negative Acknowledge)\n";
