...
if [ -n "$OFFERED_IPV4_ADDRESS" ]; then
	$SCRIPT_DEPENDENCY_PATH_CHECK_IPV4_ADDRESS_IS_VALID --address $OFFERED_IPV4_ADDRESS
	case $? in
		0) ;;
		1) printf "$0: the client mac address is invalid.\n"; exit 2; ;;
		*) printf "$0: script dependency failure: \"$SCRIPT_DEPENDENCY_PATH_CHECK_IPV4_ADDRESS_IS_VALID\" produced a failure exit code.\n"; exit 3 ;;
...
echo "\t#Flags: broadcast flag is enabled for DHCPOFFER";
echo "\t\t@ih,80,1 1 \\\\";
...
echo "\t#DHCP Message Type of 2 (Offer)";
