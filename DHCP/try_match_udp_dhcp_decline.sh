...
print_description() {
	printf "A program that prints part of an NFT rule 'match' section. The match intends to identify DHCP Decline packets.\n">&2;
}
...
print_usage() {
	...
	printf " Note: without this restriction, DHCP Decline packets are permitted to any program."
	printf "\n">&2;
...
if [ $SKIP_VALIDATION -eq 0 ]; then
	...
	if [ -n "$REQUESTED_ADDRESS_IPV4" ]; then
		$SCRIPT_DEPENDENCY_PATH_CHECK_IPV4_ADDRESS_IS_VALID --address $REQUESTED_ADDRESS_IPV4
		case $? in
			0) ;;
			1) printf "\nInvalid --requested-address-ipv4. "; print_usage_then_exit; ;;
			*) printf "$0: dependency \"$SCRIPT_DEPENDENCY_PATH_CHECK_IPV4_ADDRESS_IS_VALID\" produced a failure exit code.\n"; exit 3 ;;
		esac
	fi
...
printf "\\t#See RFC1541 - DHCP DECLINE should be 0 sec\n";
printf "\\t\\t@ih,64,16 0 \\\\\n";

printf "\\t#Flags: no flags for DHCP DECLINE, 16 zeroes, DHCPDECLINE is not broadcasted (See RFC1541)\n";
printf "\\t\\t@ih,80,16 0 \\\\\n";

printf "\\t#CIADDR (Client IP Address)\n";
printf "\\t#DHCPDECLINE does not require a client IP address\n";
printf "\\t\\t@ih,96,32 0 \\\\\n";
...
printf "\\t#DHCP Message Type of 4 (Decline)\n";
