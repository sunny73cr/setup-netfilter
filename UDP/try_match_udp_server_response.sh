...
DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_MIDDLE="./UDP/try_match_udp_session_middle.sh";

if [ ! -x "$DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_MIDDLE" ]; then
	echo "$0; script dependency failure: \"$DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_MIDDLE\" is missing or is not executable.">&2;
	exit 3;
fi
...
if [ -n "$SERVER_MAC" ]; then
	CMD_TRY_MATCH_ETHERNET_HEADER="$CMD_TRY_MATCH_ETHERNET_HEADER --source-mac-address $SERVER_MAC";
fi
if [ -n "$CLIENT_MAC" ]; then
	CMD_TRY_MATCH_ETHERNET_HEADER="$CMD_TRY_MATCH_ETHERNET_HEADER --destination-mac-address $CLIENT_MAC";
fi
...
case "$IP_VERSION" in
	"4")
		...
		CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --protocol-id $PROTOCOL_ID";
		if [ -n "$CLIENT_IP" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --source-ipv4-address $SERVER_IP";
		elif [ -n "$CLIENT_NETWORK" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --source-ipv4-network $SERVER_NETWORK";
		fi
		if [ -n "$SERVER_IP" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --destination-ipv4-address $CLIENT_IP";
		elif [ -n "$SERVER_NETWORK" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --destination-ipv4-network $CLIENT_NETWORK";
		fi
...
CMD_TRY_MATCH_UDP_HEADER="$DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_MIDDLE";
if [ -n "$CLIENT_PORT" ]; then
	CMD_TRY_MATCH_UDP_HEADER="$CMD_TRY_MATCH_UDP_HEADER --source-port $SERVER_PORT";
fi
if [ -n "$SERVER_PORT" ]; then
	CMD_TRY_MATCH_UDP_HEADER="$CMD_TRY_MATCH_UDP_HEADER --destination-port $CLIENT_PORT";
fi
if [ -n "$SERVICE_UID" ]; then
	CMD_TRY_MATCH_UDP_HEADER="$CMD_TRY_MATCH_UDP_HEADER --service-user-id $SERVICE_UID";
fi
PROTOCOL_MATCH=$($CMD_TRY_MATCH_UDP_HEADER);
check_success;
