...
DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_START="./UDP/try_match_udp_session_start.sh";

if [ ! -x "$DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_START" ]; then
	echo "$0; script dependency failure: \"$DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_START\" is missing or is not exectuable.";
	exit 3;
fi
...
CMD_TRY_MATCH_UDP_HEADER="$DEP_SCRIPT_PATH_TRY_MATCH_UDP_SESSION_START";
