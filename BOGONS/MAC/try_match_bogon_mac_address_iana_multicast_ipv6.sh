...
print_usage() {
	...
	printf "Parameters:\n">&2;
	printf "\n">&2;
	printf "  Optional: --argument string (describe a required format).\n">&2;
	printf "  Required: --argument string (describe a required format).\n">&2;
	printf "\n">&2;
}
...
print_description() {
	printf "A program that prints a portion of an NFT rule match section. The match identifies packets with a source or destination MAC (ether) address of XX:XX (Type).\n">&2;
}
...
echo "\t\tether $MAC_ADDRESS_TYPE XX:XX \\";
