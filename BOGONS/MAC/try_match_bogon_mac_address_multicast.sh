...
print_description() {
	printf "A program that prints a portion of an NFT rule match section. The match identifies packets with a source or destination MAC (ether) address where the first bit within the first octet is set (Multicast MAC Addresses).\n">&2;
}
...
printf "\\t\\tether $MAC_ADDRESS_TYPE & 01:00:00:00:00:00 != 00:00:00:00:00:00 \\\\\n";
