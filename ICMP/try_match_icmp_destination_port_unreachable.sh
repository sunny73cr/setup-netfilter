...
case "$IP_VERSION" in
	"4")
		...
		if [ -n "$CLIENT_IP_ADDRESS" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --source-ipv4-address $CLIENT_IP_ADDRESS";
		elif [ -n "$CLIENT_IP_NETWORK" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --source-ipv4-network $CLIENT_IP_NETWORK";
		fi
		if [ -n "$SERVER_IP_ADDRESS" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --destination-ipv4-address $SERVER_IP_ADDRESS";
		elif [ -n "$SERVER_IP_NETWORK" ]; then
			CMD_TRY_MATCH_IPV4_HEADER="$CMD_TRY_MATCH_IPV4_HEADER --destination-ipv4-network $SERVER_IP_NETWORK";
		fi
