...
print_description() {
	printf "A program that prints a portion of an NFT rule match section. The match identifies packets with a source or destination IPV4 address of 192.0.0.171 (ipv6 to ipv4 (NAT) DNS server discovery).\n">&2;
}
...
printf "\\t\\tip $IPV4_ADDRESS_TYPE 192.0.0.171 \\\\\n";
