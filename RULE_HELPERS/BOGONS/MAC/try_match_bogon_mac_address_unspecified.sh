...
print_description() {
	printf "A program that prints a portion of an NFT rule match section. The match identifies packets with a source or destination MAC (ether) address where no bits are set (\"Unspecified\" or \"empty\" MAC addresses).\n">&2;
}
...
printf "\\t\\tether $MAC_ADDRESS_TYPE 00:00:00:00:00:00 \\\\\n";
