...
print_description() {
	printf "A program that prints a portion of an NFT rule match section. The match identifies packets with a source or destination MAC (ether) address of FF:FF:FF:FF:FF:FF (Broadcast).\n">&2;
}
...
printf "\\t\\tether $MAC_ADDRESS_TYPE FF:FF:FF:FF:FF:FF \\\\\n";
