...
print_description() {
	printf "A program that prints part of an NFT rule 'match' section. The match intends to match NTP Kiss Of Death / Control Channel Messages.\n">&2;
}
...
if [ $SKIP_VALIDATION -eq 0 ]; then
	...
	if [ -n "$ROOT_DELAY_RANGE" ]; then
		if [ -z "$(echo $ROOT_DELAY_RANGE | grep '[0-9]\{1,4\}-[0-9]\{1,4\}')" ]; then
	...
	if [ -n "$ROOT_DISPERSION_RANGE" ]; then
		if [ -z "$(echo $ROOT_DISPERSION_RANGE | grep '[0-9]\{1,4\}-[0-9]\{1,4\}')" ]; then
...
if [ -n "$REFERENCE_ID" ]; then
	...
	case $REFERENCE_ID in
		DENY) REFERENCE_ID_DECIMAL=1145392729; ;;
		RSTR) REFERENCE_ID_DECIMAL=1381192786; ;;
		RATE) REFERENCE_ID_DECIMAL=1380013125; ;;
		ACST) REFERENCE_ID_DECIMAL=1094931284; ;;
		AUTH) REFERENCE_ID_DECIMAL=1096111176; ;;
		AUTO) REFERENCE_ID_DECIMAL=1096111183; ;;
		BCST) REFERENCE_ID_DECIMAL=1111708500; ;;
		CRYP) REFERENCE_ID_DECIMAL=1129470288; ;;
		DROP) REFERENCE_ID_DECIMAL=1146244944; ;;
		INIT) REFERENCE_ID_DECIMAL=1229867348; ;;
		MCST) REFERENCE_ID_DECIMAL=1296257876; ;;
		NKEY) REFERENCE_ID_DECIMAL=1313555801; ;;
		RMOT) REFERENCE_ID_DECIMAL=1380798292; ;;
		STEP) REFERENCE_ID_DECIMAL=1398031696; ;;
	esac
fi
...
printf "\\t#Match NTP Mode (4 - server response)\n";
printf "\\t\\t@$OFFSET_MARKER,$BIT_OFFSET_MODE,3 4 \\\\\n";

printf "\\t#Match NTP Stratum 0\n";
printf "\\t\\t@$OFFSET_MARKER,$BIT_OFFSET_STRATUM,8 0 \\\\\n";
...
#Not a part of NTP Kiss of Death / Control Channel Packets
